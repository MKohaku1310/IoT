 ...
 * Chạy: node generate_docs.js
 * Output: ../TEST_COMMANDS.docx
 */
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      ...labeledCode('Lắng nghe sự kiện UPDATE bảng den trong 30 giây', [
        'node -e "',
        "const { createClient } = require('@supabase/supabase-js');",
        "const sb = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);",
      ...
      ...labeledCode('Bật Đèn (idden=3, trangthai=1)', [
        'node -e "',
        "const { createClient } = require('@supabase/supabase-js');",
        "const sb = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);",
        "sb.from('den').update({trangthai:1}).eq('idden',3).select()",
      ...
      ...codeBlock([
        'SELECT l.idluat, d.tenden, l.loaicambien, l.toantu,',
        '       l.nguong, l.automation',
        'FROM luat l',
        'JOIN den d ON d.idden = l.idden',
        'ORDER BY l.idluat;',
      ]),
      spacer(),
      ...
      makeTable(
        ...
        [
          ...
          ['Backend không nhận event từ Web', 'Supabase Realtime chưa bật', 'Chạy lại schema.sql phần ALTER PUBLICATION'],
...
// ─── Xuất file ────────────────────────────────────────────────────────────────
const outputPath = path.join(__dirname, '..', 'TEST_COMMANDS.docx');
