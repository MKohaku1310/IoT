...
function NotifItem({
  ...
  notif: { id: any; ts: number; detail: string; level: "error" | "warn" | "info"; isoTime: string; isBroadcast: boolean };
  isRead: boolean;
  onMark: () => void;
}) {
...
export function NotificationsTab({
  ...
  useEffect(() => {
    async function fetchAll() {
      ...
      try {
        ...
        if (currentUser?.idnguoidung) {
          // Lấy nhật ký thao tác, cảnh báo hệ thống, và thông báo Admin gửi cho User ('admin_notification')
          // KHÔNG lấy 'user_to_admin' (vì đây là yêu cầu User gửi đi cho Admin, không phải thông báo nhận về)
          query = query.or(`idnguoidung.eq.${currentUser.idnguoidung},idnguoidung.is.null`);
        }

        query = query.in("loai_thongbao", ["user_action", "system_alert", "admin_notification"]);

        const { data } = await query.limit(100);
        if (data) {
          const cleanLogs = data.filter((l) => !(l.hanhdong.includes("Ánh sáng") && l.hanhdong.includes("dưới ngưỡng")));
          setLogs(cleanLogs);
          const readIdsFromDb = cleanLogs.filter((l) => l.da_doc === true).map((l) => Number(l.idnhatky));
          if (readIdsFromDb.length > 0) {
            onMarkAllAsRead(readIdsFromDb);
          }
        }
      } catch (e) {
    ...
    const chan = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "nhatkyhoatdong" }, (payload) => {
        ...
        // Chỉ hiển thị nếu không phải user_to_admin và thuộc về User hoặc Broadcast
        if (record.loai_thongbao !== 'user_to_admin') {
          const isForMe = record.idnguoidung === null || Number(record.idnguoidung) === Number(currentUser?.idnguoidung);
          const isLightSpam = record.hanhdong?.includes("Ánh sáng") && record.hanhdong?.includes("dưới ngưỡng");
          if (isForMe && !isLightSpam) {
            setLogs((prev) => [record, ...prev].slice(0, 100));
          }
        }
      })
      .subscribe();
  ...
  const mapped = useMemo(() => logs.map((l) => {
    ...
    const isBroadcast = l.idnguoidung === null && l.idnode === null;
    try {
      const parsed = JSON.parse(l.hanhdong);
      detail = parsed.description || l.hanhdong;
      if (parsed.loai_nhatky === "system_alert") {
        const isCritical = (parsed.description || "").includes("Lỗi") || (parsed.description || "").includes("Mất kết nối");
    ...
    return { id: Number(l.idnhatky), ts: Date.parse(l.thoigian), detail, level, isoTime: l.thoigian, isBroadcast, daDoc: l.da_doc === true };
  }), [logs]);
          ...
          <ul className="divide-y divide-slate-100/80">
            {paginatedData.map((n) => (
              <NotifItem
                key={n.id}
                notif={n}
                isRead={readAlertIds.includes(n.id) || !!n.daDoc}
