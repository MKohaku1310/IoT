...
export function HealthTab({
  ...
  useEffect(() => {
    ...
    unsubs.push(mqttSubscribe("buivansang_iot_pj/s3-node-01/uptime", (payload) => {
      const val = parseInt(payload, 10);
      if (!isNaN(val)) setRawUptime(val);
    }));

    unsubs.push(mqttSubscribe("buivansang_iot_pj/s3-node-01/rssi", (payload) => {
      const val = parseInt(payload, 10);
      if (!isNaN(val)) setRssi(val);
    }));

    unsubs.push(mqttSubscribe("buivansang_iot_pj/s3-node-01/latency", (payload) => {
      const val = parseInt(payload, 10);
      if (!isNaN(val)) setLatency(val);
    }));
  ...
  useEffect(() => {
    const fetchStorage = async () => {
      try {
        const { data, error } = await supabase.storage.getBucket("sensor-data");
        if (!error && data) {
          const bkt = data as any;
          const used = bkt.file_size ?? 0;
          const total = bkt.size_limit ?? 524288000;
          setStorage({ used: Math.round(used / 1048576), total: Math.round(total / 1048576) });
        } else {
          setStorage({ used: 0, total: 500 });
        }
      } catch {
        setStorage({ used: 0, total: 500 });
      }
    ...
    fetchStorage();
    const id = setInterval(fetchStorage, 60000);
    return () => clearInterval(id);
  }, []);

  const rssiPct = rssi != null ? Math.max(0, Math.min(100, ((rssi + 90) / 50) * 100)) : 0;
  const rssiLevel = rssi == null ? "Chưa có dữ liệu" : rssi > -60 ? "Xuất sắc" : rssi > -70 ? "Tốt" : rssi > -80 ? "Trung bình" : "Yếu";
  const latencyLevel = latency == null ? { l: "Chưa có dữ liệu", c: "text-slate-500", bg: "bg-slate-100" } : latency < 100 ? { l: "Ổn định", c: "text-emerald-600", bg: "bg-emerald-100" } : latency < 200 ? { l: "Chậm nhẹ", c: "text-amber-600", bg: "bg-amber-100" } : { l: "Trễ cao", c: "text-rose-600", bg: "bg-rose-100" };
  const storagePct = storage ? (storage.used / storage.total) * 100 : 0;

  const [latHist, setLatHist] = useState<{ t: number; latency: number }[]>([]);
  useEffect(() => {
    if (latency == null) return;
    setLatHist(prev => {
      const next = [...prev, { t: Date.now(), latency }];
  ...
  const [hwInfo, setHwInfo] = useState<Record<string, string>>({});

  useEffect(() => {
    const unsubs: (() => void)[] = [];
    const topics: [string, string][] = [
      ["buivansang_iot_pj/s3-node-01/firmware", "Firmware"],
      ["buivansang_iot_pj/s3-node-01/chip", "Chip"],
      ["buivansang_iot_pj/s3-node-01/ip", "Địa chỉ IP"],
      ["buivansang_iot_pj/s3-node-01/mac", "MAC"],
    ];
    topics.forEach(([topic, label]) => {
      unsubs.push(mqttSubscribe(topic, (payload) => {
        const val = payload?.trim();
        if (val) setHwInfo((prev) => ({ ...prev, [label]: val }));
      }));
    });
    return () => unsubs.forEach((u) => u());
  }, []);

  const defaultHw: Record<string, string> = {
    Firmware: "v2.4.1",
    Chip: "ESP32-S3 · Xtensa LX7 dual-core 240MHz",
    RAM: "512 KB SRAM · 2 MB PSRAM",
    Flash: "8 MB",
  };
  const displayHw = { ...defaultHw, ...hwInfo };
    ...
    <div className="space-y-6">
      <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-4">
        ...
        <HealthStat
          icon={Wifi}
          label="WiFi RSSI"
          value={rssi != null ? `${Math.round(rssi)} dBm` : "—"}
          hint={rssiLevel}
        ...
        <HealthStat
          ...
          value={storage ? `${storage.used} / ${storage.total} MB` : "—"}
          hint={storage ? `Đã dùng ${storagePct.toFixed(0)}%` : "Đang tải..."}
      ...
      <GlassCard>
        <div className="mb-4">
          ...
          <p className="text-xs text-slate-500">Dữ liệu real-time từ MQTT broker</p>
        </div>
        <div className="h-56 w-full">
          ...
          <ResponsiveContainer width="100%" height="100%">
            <AreaChart data={latHist} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              <defs>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <GlassCard>
          ...
          <div className="mt-3 space-y-2 text-sm text-slate-700">
            {Object.entries(displayHw).map(([k, v]) => (
              <HwRow key={k} k={k} v={v} />
            ))}
          </div>
        </GlassCard>
